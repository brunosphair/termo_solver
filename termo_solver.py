...
class Solver:
            ...
            for word in self.all_possible_words:
                ...
                for y in range(1, final):
                    if sorted_letters[(-1) * y] in unidecode(word):
                        count += 1
                    if unidecode(word[y - 1]) in self.right_letters:
                        count -= 1
    ...
    def classes_analyse(self, word, classes):  # Receives the tried word and a list ('all_results') that
        ...
        for n in range(5):
            ...
            if result != -1:
                if result == 'letter wrong':
                    self.delete_letter(unidecode(word[n]))
                elif result == 'letter place':
                    self.include_letter(unidecode(word[n]), n + 1)
                    self.right_letters.add(word[n])
                elif result == 'letter right':
                    self.include_letter_pos(unidecode(word[n]), n + 1)
                    self.right_letters.add(word[n])
...
def load_txt():  # Loads the txt from lexico pt-br and put the words with 5 letters in a list
    ...
    with open('palavras.txt', encoding='utf8') as f:
        for item in f:
            words.append(item.rstrip().upper())  # TODO: add unidecode here
